...
export class PokemonApiService {
   ...
   * @returns A promise that resolves with the generated Pokemon object, ready for DB insertion.
   * @throws {Error} if the API call fails or returns an error.
   */
  public async generatePokemon(): Promise<Omit<Pokemon, 'id'>> {
    ...
    try {
      ...
      if (!data || !data.imageBase64 || !data.metadata || !data.metadata.name || !data.metadata.rarity || !data.generatedAt) {
      ...
      const pokemonData: Omit<Pokemon, 'id'> = {
        ...
        prompt: `A pokémon of rarity ${data.metadata.rarity} named ${data.metadata.name}.`,
